function [x,F] = scBFGS(obj,x,dmax,a0,a1,eta,theta,C,rho,tau,M)
...
while d < dmax
  ...
  % Compute stochastic gradient
  [g,d] = stoch_grad(obj,x,d);
  
  % Compute norm of subgradient
  norms.g = norm(g);
  
  % Compute independent estimate
  if C > 0, [ghat,d] = stoch_grad(obj,x,d); end
  ...
  while 1
    ...
    % Get betas
    betas = beta_vals(s,a*y,eta,theta);
    ...
    % Compute SG
    [g_temp   ,d] = stoch_grad(obj,x,d);
    [ghat_temp,d] = stoch_grad(obj,x,d);
...
function [g,d] = stoch_grad(obj,x,d)
...
function betas = beta_vals(s,y,eta,theta)
